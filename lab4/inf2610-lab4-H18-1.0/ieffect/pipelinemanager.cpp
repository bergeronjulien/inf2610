...
void PipelineManager::launchParallel(int queueSize)
{
    ...
    int size = m_stageList.size();

    HANDLE threadArray[size];
    DWORD threadID[size];


    for(int i = 0; i < m_stageList.size(); i++){
        threadArray[i] = CreateThread(NULL, 0, startHelper, (LPVOID)m_stageList.at(i),0,&threadID[i]);
    }

    WaitForMultipleObjects(m_stageList.size(), threadArray, TRUE, INFINITE);
    for(int i = 0; i < m_stageList.size(); i++){
